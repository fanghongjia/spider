...
from bingfeng.items import BingfengItem  # 报错是pyCharm的原因

reload(sys)
sys.setdefaultencoding('utf-8')

class bingfengSpider(scrapy.spiders.Spider):
    name = "bingfeng"  # 在命令行中输入scrapy crawl bbs125启动此爬虫
    allowed_domains = ["bingfeng.tw"]  # 非此域名的url不爬
    cookies = {
    ...
    start_urls = [
        "https://bingfeng.tw/thread-444191-1-1.html"  # 452326
    ]
    ...
    def parse(self, response):
        ...
        for each in table:
            ...
            try:
                item['author'] = each.xpath('tr[1]/td[@class="pls"]/div[@class="pi"]/div[@class="authi"]/a/text()').extract()[0]
            except:
                item['author'] = each.xpath('tr[1]/td[@class="pls"]/div[@class="pi"]/text()').extract()[0]
                continue
