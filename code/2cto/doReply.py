...
def doReply(url, domain, Cookie):
    ...
    form_div = soup.find(attrs={'class':'t'}) # 找到包含回复表单的div
    ...
    title = form_div.find(attrs={'class':'input mr10'}).get('value')[3:] # 通过数据包分析得表单里除了隐藏域还有标题和回复的信息
    form_data['atc_title'] = title
    reply = u'I really like this card!' + str(time.time())
    form_data['atc_content'] = reply
    print form_data
    nowTime = str(time.time()) # 获取当前时间戳
    form_action = "http://" + domain + "/" + form.get('action') + "&nowTime=" + nowTime + "&verify=" + form_data['verify'] # 找到表单提交的地址，即处理页地址
    form_headers = {
        'User-agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36',
        'Cookie':Cookie,
        'Connection':'keep-alive'
