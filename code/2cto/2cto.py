...
spiderUrl = "http://bbs.2cto.com/thread.php?fid=69&page="
# 入库存储的板块名称
board = "杀毒软件专区"
# 该板块内需要爬取的起始页号
page_start = 28
...
for k in range(page_start, page_end):
    ...
    for title in tagA:
        ...
        try: 
            ...
            print "bad url!"
            continue
        out_time = soup2.find(attrs={'class':'tipTop s6'})
        out_time = compiled_time.search(str(out_time)).group()
        year = int(out_time[:4])
        month = int(out_time[5:7])
        print year,month
        hidden = soup2.find(attrs={'class':'f12 quoteTips'})
        if(hidden != None and (year == 2016 or (year == 2015 and month > 7))):
